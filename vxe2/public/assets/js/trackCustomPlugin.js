(function () {
  ...
  InlineChangeEditor.prototype = {
    ...
    startTracking: function () {
      ...
      // Safari gives an error at this point, but nothing is broken (at least at first sight): [Error] Blocked a frame with origin "http://{vxeDomain}" from accessing a frame with origin "http://{lanstadDomain}". Protocols, domains, and ports must match.
      // Is a bug from Safari that must be fixed in following releases.
    ...
    placeholdDeletes: function () {
      ...
      ice.dom.each(ice.dom.find(this.element, deleteSelector), function (
        i,
        el
      ) {
    ...
    deleteContents: function (right, range) {
      ...
      if(trackOtherContent){
        if(delType === "Caret" && right === true){
          ...
          var prevElemDel = range.startContainer.parentElement.previousSibling;
          let prevNodeName = range.startContainer.parentElement.previousSibling.nodeName;
          if(!(!prevElemDel) && prevNodeName !== "#text"){
            range = document.createRange();
            range.setStart(prevElemDel, prevElemDel.innerText.length);
            range.collapse(true);
          } else if(!(!prevElemDel) && prevNodeName === "#text") {
            range = document.createRange();
    ...
    _deleteSelection: function (range) {
      ...
      for (var i = 0; i < elements.length; i++) {
        ...
        if (
          elem.nodeName == 'PAGENO' ||
          elem.nodeName == 'PAGENOR' ||
          elem.nodeName == 'LH' ||
          elem.nodeName == 'RH' ||
          elem.nodeName == 'PAGENOB' ||
          elem.nodeName == 'FOOTNOTE1'
        ) {
          continue;
        }
    ...
    _handleAncillaryKey: function (e) {
      ...
      switch (key) {
        case ice.dom.DOM_VK_DELETE:
          ...
          $('#character_count').val(html_count);
          $('#backspace_key').val(1);
          console.log('backspace key count :: ' + html_count);
          break;
        case 46:
    ...
    _handleEnter: function () {      
      ...
      var parent_node = affnodeas.parentElement;
      var grand_parent = parent_node.parentElement.tagName;

      var tag_parent = affnodeas.parentElement.tagName;

      console.log('tag_name :' + tag_name);
      console.log('tag_parent :' + tag_parent);
      console.log('tag_grandparent :' + grand_parent);
      ...
      if (
        ...
        // while click the enter button add another para after the selection para (go to line no:402)
        return true;
      } else {
        return false;
      }
