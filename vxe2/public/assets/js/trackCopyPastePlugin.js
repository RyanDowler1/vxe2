(function () {
  ...
  IceCopyPastePlugin.prototype = {
    ...
    keyDown: function (e) {
      if(e.ctrlKey && e.keyCode == 86 && $('#myModalfront').is(':visible')) {           
        let i=0;
        document.querySelector("#form_fm").addEventListener("paste", function(e) {    
          ...
          let dt = Date.parse(d);
          e.preventDefault();
          var pasteTxt = e.clipboardData.getData("text/plain");          
          document.execCommand("insertHTML", false, '<span class="ins cts-1" data-cid="2" data-userid='+userId+' data-username="'+userName+'" data-time="'+ dt +'">'+pasteTxt+'</span>');          
          i =i+1;
          }          
        });
        return;        
      ...
      if (ice.dom.hasBlockChildren(fragment)) {
        ...
        while (fragment.firstChild) {
          if (
            ...
            !jQuery.trim(fragment.firstChild.nodeValue)
          ) {
    ...
    htmlDecode: function (content) {
      ...
      var pasteCon = $(e).html(content);
      if ($(pasteCon[0]).html().split('&lt;').length > 1) {
        pasteCon = pasteCon[0].innerText;
        var pasteCon2 = $(e).html(pasteCon);
        if ($(pasteCon2[0]).html().split('&lt;').length > 1) {
          pasteCon = pasteCon2[0].innerText;
        }
      } else pasteCon = pasteCon[0].innerHTML;
      return pasteCon;
    },

    removeStyleTagcontent: function (content) {
      // Whoever is working in LV-432:
      // Despite being a part of the standard since 2018, new regex flags like /s are not supported in Firefox and IE (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions, https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/RegExp/dotAll).
      // Due to the browser imcompatibility problem with previous "/gms" expression, the "s" flag has been removed.
      // Anyway, this expression seems to not being working in chrome/safari (not even with the "s" flag), check its structure to find the bug.
      // A solution to not using the old s in /gms is using a [\S\s] expression instead.
      var regex = /(<style[^>]*>)((?:(?!<style>).)*)(<\/style>)/gm;
      var str = content;
      var subst = `$1$3`;
    ...
    _cleanWordPaste: function (content) {
      ...
      //Remove style tag's css
      content = this.removeStyleTagcontent(content);
    ...
    _cleanPaste: function (content) {
      ...
      content = content.replace(/<b(\s+|>)/g, '<b$1');
      content = content.replace(/<\/b(\s+|>)/g, '</b$1');
      content = content.replace(/<i(\s+|>)/g, '<i$1');
      content = content.replace(/<\/i(\s+|>)/g, '</i$1');
      return content;
    },
