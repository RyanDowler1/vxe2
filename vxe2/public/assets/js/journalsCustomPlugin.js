(function () {
  ...
  InlineChangeEditor.prototype = {
    ...
    initializeRange: function () {
      ...
      //var found123 = ice.dom.find(this.element, this.blockEls.join(', '))[0];
      range.setStart(
    ...
    handleEvent: function (e) {
      ...
      if (e.type == 'mouseup') {
        ...
        if ($('#myModalfront').is(':visible'))
          window.Journals.articleFieldUpdate(e, 'keydown');
        if (e.shiftKey == true) {
          if (e.which == 222) {
            var affnodeas = getSelectedNode();
            var tag_name = affnodeas.tagName;

            var contenteditable = affnodeas.getAttribute('contenteditable');

            console.log('tag_name :' + tag_name);
            console.log('contenteditable :' + contenteditable);
            if (tag_name === 'TR') {
              contenteditable = 'true';
              // check row delete or not
              var className = affnodeas.getAttribute('class');
              if (className !== null) {
                var arrayClass = className.split(' ');
                var checkDelete = $.inArray('delete', arrayClass);
                if (checkDelete === 1) {
        ...
        if (e.ctrlKey == false) {
          ...
          //Keypress for L -76, F-70, Y-89, 1-49, 2-50, 3-51, 5-53, 6-54, 7-55, 8-56
          if (
            e.which == 76 ||
            e.which == 70 ||
            e.which == 89 ||
          ...
          if (e.which == 66) {
            ...
            if (editRefPer === false) {
              referenceElement = affnodeas.closest('mixed-citation,ref');
            }
            var selectedHTML = getHTMLOfSelection();
            if (selectedHTML !== '') {
              var checkvalidation = validateFootNote(selectedHTML);
              if (checkvalidation === false) {
                return false;
              }
            }
            var curTag = affnodeas.tagName;
            browser_details('Paste');
            var absElement = affnodeas.closest('abstract1');
            if (absElement === null && tag_name !== 'ABSTRACT1') {
            }
            ...
            storeFmData();
            var selObj = window.getSelection();
            browser_details('Cut :' + selObj);
            var affnodeas = getSelectedNode();
            var selectedHTML = getHTMLOfSelection();
            var deleteConfirm = $('#deleteConfirm').val();
            if (selectedHTML !== '' && deleteConfirm !== '1') {
              var checkvalidation = validateFootNote(selectedHTML);
              if (checkvalidation === false) {
                return false;
              }
            ...
            var curTag = affnodeas.tagName;
            if (curTag === 'REF' || referenceElement !== null) {
              return false;
            }
          } else if (e.which == 189) {
            var localdata_set = $('.localstorevalue').val();
        ...
        if ($('#myModalfront').is(':visible'))
          window.Journals.articleFieldUpdate(e, 'keyup');
        if (
          e.which === 37 ||
          e.which === 38 ||
          e.which === 39 ||
          ...
          window.BreadCrumbTrigger.breadCrumb();
        }
        if (
          e.which != 123 &&
          e.which != 27 &&
          ...
          if (e.which != 66 && e.which != 73 && e.which != 85) {
            msg = affnodeas.innerHTML;
            browser_details(e.key + ' (Ref: ' + msg + ')');
            if (!$('#myModalfront').is(':visible')) {
              this.r++;
              setTimeout(() => {
                this.s++;
                if (this.r === this.s) {
                  ...
                  localStorage.removeItem('keyPressData');
                  localStorage.removeItem('keyDateId');
                }
              }, 1000);
            }
          }
          var affnodeas = getSelectedNode();
          updatedRevisions(affnodeas);
        } else if (e.which == 13) {
          // var affnodeas = getSelectedNode();
          ...
          // console.log("tag_enter1 :"+tag_name);
          // console.log("tag_enter1 :"+tag_parent);

          // if(tag_name == "PP1" && tag_parent != "LI")
          // {
          pasteHtmlAtCaret('<span class="enter_keypressed"></span>');
    ...
    insert: function (node, range) {
      ...
      if (editRefPer === false) {
        refElement = affnodeas.closest('ref');
      }
      var absElement = affnodeas.closest('abstract1');
      if (absElement === null && tag_name !== 'ABSTRACT1') {
      }
      if (refElement !== null || tag_name === 'REF-LIST') {
        contenteditable = 'false';
      }
      if (
        tag_name == 'FOOTNOTE1' ||
      ...
      //Adding new span tag for editing other user content - MV
      var currentUser = this.currentUser.name;
      var contentOwnName =
        this.selection._selection.focusNode.parentNode.dataset.username;
      var currentDomId =
        this.selection._selection.focusNode.parentNode.dataset.time;
      // end MV
      // If we have any nodes selected, then we want to delete them before inserting the new text.
      if (!range.collapsed) {
        //Added by MV
        if (currentUser == contentOwnName) this.deleteContents();
        else {
          var delKey = true;
          this.initDeleteOtherContent(currentUser, delKey);
        }
        // end MV
        // Update the range
      ...
      // Added by MV
      if ($(range.startContainer).attr('data-username'))
        contentOwnName = $(range.startContainer).attr('data-username');
      if (currentUser != contentOwnName && contentOwnName != undefined) {
        //  this.editingOtherUserContent(currentUser, contentOwnName);
        this.modifyOtherContent(range, currentUser);
        setTimeout(function () {
        ...
        if (
          event.key == ' ' &&
          this.selection._selection.focusNode.parentElement.nodeName ==
            'SPAN' &&
          currentDomId != undefined
        )
          this.joinOwnInsSpace(currentDomId, focusOffset);
        return isPropagating;
      }
      // return isPropagating;
      // end MV
    ...
    modifyOtherContent: function (range, currentUser) {
      var currentId = this.currentUser.id;
      var getDate = new Date(),
        dataTime = Date.parse(getDate);
      var getFormatedTime = this.getFormatedTime(new Date()),
        getFormatedDate = this.getFormatedDate();
      let spanTag = document.createElement('SPAN');
      this.setAttributes(spanTag, {
        class: 'ins cts-2',
    ...
    joinOwnInsSpace: function (currentDomId, focusOffset) {
      var removeSpace = $(
        '#content .ins[data-time="' + currentDomId + '"]'
      ).text();
      $('#content .ins[data-time="' + currentDomId + '"]').text(removeSpace);
      ...
      if (el.childNodes != null || el.childNodes != undefined) {
    ...
    deleteContents: function (right, range) {
      ...
      const delType = window.getSelection().type;
      ...
      //Place the cursor on prev or next element MV 
      var trackOtherContent = true;
      let checkOtherContent = range.startContainer.parentElement.nodeName;      
      if (checkOtherContent !== 'SPAN')
        trackOtherContent = false;
      if (trackOtherContent) {
        if (delType === 'Caret' && right === true) {
          var nextElemDel = range.startContainer.parentElement.nextSibling;
          if (!(!nextElemDel)) {
            range = document.createRange();
            range.setStart(nextElemDel, 0);
            range.collapse(true);
          }
        } else {
          let prevElemDel = range.startContainer.parentElement.previousSibling;
          if (!(!prevElemDel)) {
            let prevNodeName =
              range.startContainer.parentElement.previousSibling.nodeName;
            if (prevNodeName !== '#text') {
              // range = document.createRange();              
              // range.setStart(prevElemDel, prevElemDel.innerText.length);
              range.setStartAfter(prevElemDel);
              range.collapse(true);
            } else if (prevNodeName === '#text') {
              // range = document.createRange();
              // range.setStart(prevElemDel, prevElemDel.length);
              range.setStartAfter(prevElemDel);
              range.collapse(true);
            }
          } else {
            $(range.startContainer.parentElement).attr(
              'contenteditable',
              false
            );
            let prePareElemDel =
            ...
            range = document.createRange();
            range.setStart(prePareElemDel, 0);
            range.collapse(true);
          }
        }
      }
    ...
    addNodeToChange: function (changeid, ctNode) {
      ...
      ctNode.setAttribute('data-user', 'current-user');
    ...
    _insertNode: function (node, range, insertingDummy) {
      ...
      if (ice.dom.hasNoTextOrStubContent(parentBlock)) {
        ...
        if (
          parentBlock.tagName === 'PP1' &&
          (parentBlock.getAttribute('enter_key') === 'success' ||
            parentBlock.getAttribute('new-elements') === 'Type the Para Here')
        ) {
          //No need break tag for para while enter
        } else {
      ...
      //Adding new span tag for editing other user content - MV
      var currentUser = this.currentUser.name;
      if (this.selection._selection.focusNode.parentNode != null)
        var contentOwnName =
          this.selection._selection.focusNode.parentNode.dataset.username;
      else var contentOwnName = undefined;
      if (
        (insertingDummy && inCurrentUserInsert) ||
        (currentUser != contentOwnName && contentOwnName != undefined)
      )
        return;
      //end - MV
    ...
    _deleteSelection: function (range) {
      ...
      for (var i = 0; i < elements.length; i++) {
        ...
        if (elemNodeName === 'QUERY' || elemNodeName === 'COMMENT') {
          elem.remove();
          continue;
        }
    ...
    _addNodeTracking: function (contentNode, range, moveLeft) {
      ...
      if (
        prevDelNode &&
        this._currentUserIceNode(prevDelNode) &&
        !$(prevDelNode).hasClass('del-key')
      ) {
        // Added by MV
        ctNode = prevDelNode;
        ...
        nextDelNode &&
        this._currentUserIceNode(nextDelNode) &&
        !$(nextDelNode).hasClass('del-key')
      ) {
        // Added by MV
        ctNode = nextDelNode;
    ...
    _handleAncillaryKey: function (e) {
      ...
      switch (key) {
        case ice.dom.DOM_VK_DELETE:
          ...
          storedata(localdata_set);
          storeFmData();
          //Add new span tag for deleting other user content - MV
          preventDefault = this.initDeleteOtherContent(currentUser);
        ...
        case 46:
          ...
          storeFmData();
          var selObj = window.getSelection();
          //browser_details("Deleted :"+selObj);
    ...
    _handleEnter: function () {
      ...
      console.log('tag_enter :' + tag_name);
      console.log('tag_enter :' + tag_parent);
    ...
    // Inserting content from current user in another user content - MV
    editingOtherUserContent: function (currentUser, contentOwner) {
      var currSelection = this.selection._selection;
      var currentId = this.currentUser.id;
      var getDate = new Date(),
        dataTime = Date.parse(getDate);
      var getFormatedTime = this.getFormatedTime(new Date()),
      ...
      var typedTxt = event.key;
      var focusOffset = window.getSelection().focusOffset;

      var currentDomTitle =
        currSelection.anchorNode.parentNode.attributes.title.nodeValue;
      var wholeText = currSelection.anchorNode.wholeText;
      var textTOArr = Array.from(wholeText);
      textTOArr.splice(focusOffset, 0, '/' + typedTxt + '/');
      var newTagContent = textTOArr.join('');
      var firstContent = newTagContent.split('/')[0],
        secondContent = newTagContent.split('/')[2];
      //If entered space
      if (typedTxt == ' ') {
        typedTxt = '&nbsp;';
        secondContent = $.trim(secondContent);
      }
      var currentDomTime = currSelection.focusNode.parentNode.dataset.time;
      if (currentDomTime == undefined)
        currentDomTime = currSelection.focusNode.dataset.time;
      var contentSelector = $(
        '#content .ins[data-time="' + currentDomTime + '"]'
      );
      var i = 1;
      var newTime = parseInt(dataTime) - i++;

      const insOpenElement =
        "<span class='ins cts-1 ins-key' data-user='current-user' id='current-editor-" +
        dataTime +
        "' data-userid='" +
        currentId +
        "' data-username='" +
        ...
        "' data-time='" +
        dataTime +
        "' contenteditable='true' title='Inserted by " +
        currentUser +
        ' - ' +
        getFormatedDate +
        ' ' +
        getFormatedTime +
        "'>";
      const insCloseElement = '</span>';
      const insLastElement =
        ...
        secondContent +
        '</span>';
      var newEnterTxt = insOpenElement + typedTxt + insLastElement;
      var newEnterTxt2 = insOpenElement + typedTxt + insCloseElement;

      contentSelector.text(firstContent);
      var currentSpanID;
      if (firstContent.length == 0) {
        contentSelector.before(newEnterTxt2);
        contentSelector.text(secondContent);
        currentSpanID = contentSelector.prev().attr('id');
      } else if (secondContent.length != 0) {
        contentSelector.after(newEnterTxt);
        currentSpanID = contentSelector.next().attr('id');
      } else {
        contentSelector.after(newEnterTxt2);
        currentSpanID = contentSelector.next().attr('id');
      }
    ...
    // While user to delete other owner content from there account that changes will be tracking with separte span by using this method - MV
    initDeleteOtherContent: function (currentUser, delKey) {
      if (delKey) return this.deleteContents(true);
      else return this.deleteContents();
    },
    ...
    setCursorPosition: function (
      ...
      var el = document.getElementById(currentSpanID);
      var range = document.createRange();
      var sel = window.getSelection();
      ...
      if (insKey)
        ...
        //For Delete and backspace Key
        if (delKey) {
          if (el.nextSibling.childNodes[0] != undefined)
            range.setStart(el.nextSibling.childNodes[0], 0);
          else range.setStart(el.nextSibling, 0);
        } else {
          if (multiSelect) focusOffset = beforeData.length;
          if (el.previousSibling.childNodes[0] != undefined)
            range.setStart(el.previousSibling.childNodes[0], focusOffset);
          else range.setStart(el.previousSibling, 0);
          // range.setStart(el.previousSibling,focusOffset);
        }
        if (beforeData.length == 0 || endContainer) contentSelector.remove();
      }
